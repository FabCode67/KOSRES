...
import {
  Menu, X, Phone, Mail,
  TrendingUp, ShieldCheck, Home, LayoutGrid,
...
const services = [
  ...
  { label: "Other Services",  href: "/services/other",               icon: LayoutGrid,  color: "#2D4A1E" },
]
    ...
    <header className="fixed top-0 right-0 left-0 z-50 bg-white shadow-sm border-b border-border">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 flex h-16 items-center justify-between border-b border-border/50">
        <Link href="/" className="shrink-0">
          <Image src={LOGO} alt="KOSRES LTD" width={200} height={90} priority className="h-14 w-auto object-contain" />
        </Link>

        <nav className="hidden md:flex items-center gap-1">
          <Link href="/properties"
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${pathname === "/properties" ? "text-[oklch(0.42_0.19_25)] bg-red-50" : "text-slate-600 hover:text-[oklch(0.42_0.19_25)] hover:bg-muted"}`}>
            Properties
          </Link>
          <Link href="/publications"
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${pathname.startsWith("/publications") ? "text-[oklch(0.42_0.19_25)] bg-red-50" : "text-slate-600 hover:text-[oklch(0.42_0.19_25)] hover:bg-muted"}`}>
            Publications
          </Link>
          <Link href="/#about" className="px-3 py-1.5 rounded-lg text-sm font-medium text-slate-600 hover:text-[oklch(0.42_0.19_25)] hover:bg-muted transition-colors">About</Link>
          <Link href="/#contact" className="px-3 py-1.5 rounded-lg text-sm font-medium text-slate-600 hover:text-[oklch(0.42_0.19_25)] hover:bg-muted transition-colors">Contact</Link>
          <Link href="/admin" className="ml-2 px-4 py-1.5 rounded-lg bg-[oklch(0.42_0.19_25)] text-white text-sm font-semibold hover:bg-[oklch(0.36_0.18_25)] transition-colors">Admin</Link>
        </nav>
            ...
            <div className="grid grid-cols-2 gap-2">
              {services.map(({ label, href, icon: Icon, color }) => (
                <Link key={href} href={href} onClick={() => setOpen(false)}
                  ...
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-none" style={{ backgroundColor: color + "18" }}>
